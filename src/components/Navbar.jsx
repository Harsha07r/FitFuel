  ...
  return (
    <div className="w-full bg-white border-b border-gray-200 shadow-sm">
      ...
      <div className="flex justify-between items-center px-6 md:px-12 py-4">
        ...
        <h3 className="text-xl font-bold text-blue-700">FitFuel</h3>

        <ul className="hidden md:flex items-center gap-10 list-none">
          {navItems.map((item) => (
            <li
              key={item.label}
              className="text-sm text-gray-500 hover:text-blue-700 cursor-pointer font-medium transition-colors"
      ...
      <div className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-72 opacity-100' : 'max-h-0 opacity-0'}`}>
        <ul className="flex flex-col px-6 pb-4 gap-1 list-none border-t border-gray-100">
          {navItems.map((item) => (   // ✅ now uses navItems with ids
            <li
              key={item.label}
              ...
              onClick={() => scrollToSection(item.id)}  // ✅ actually scrolls now
            >
              {item.label}
            </li>
          ))}
